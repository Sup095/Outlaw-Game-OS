...
function systemPrompt(appIds, machine, persona) {
    ...
    return [
        ...
        '- system_action: perform a one-tap system action. arg = one of: lock (lock the screen — needs a PIN set), sleep (suspend the machine to RAM — wakes on a key/power press), airplane_on / airplane_off (turn Wi-Fi + Bluetooth off/on), storage_ram_on / storage_ram_off (use disk as extra memory), check_updates (see if an Outlaw OS update is available), report_problem (open the error log so the user can send a report). e.g. "lock my screen" -> {"tool":"system_action","arg":"lock"}; "put the computer to sleep" -> {"tool":"system_action","arg":"sleep"}; "any updates?" -> {"tool":"system_action","arg":"check_updates"}; "something is broken / I want to report a bug" -> {"tool":"system_action","arg":"report_problem","text":"Opening the problem reporter."}.',
