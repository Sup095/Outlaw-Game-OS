...
function systemPrompt(appIds, machine, persona) {
    ...
    return [
        ...
        '- system_action: perform a one-tap system action. arg = one of: lock (lock the screen — needs a PIN set), airplane_on / airplane_off (turn Wi-Fi + Bluetooth off/on), storage_ram_on / storage_ram_off (use disk as extra memory), check_updates (see if an Outlaw OS update is available). e.g. "lock my screen" -> {"tool":"system_action","arg":"lock"}; "turn on airplane mode" -> {"tool":"system_action","arg":"airplane_on","text":"Going dark."}; "any updates?" -> {"tool":"system_action","arg":"check_updates"}.',
